...
def test_pytorch():
  ...
  if torch.cuda.is_available():
    ...
    print("\nPerforming operations on CPU...")
    print(f"Original tensor: {test_tensor}")
    print(f"Tensor after addition: {test_tensor + 2}")
    print('Success! CPU Working!')
...
def test_ray():
  ...
  @ray.remote
  def f():
    return torch.rand(1000, 1000).sum()

  try:
    result = ray.get(f.remote())
    print('Ray can perform calculations on the CPU.')
  except RuntimeError as e:
    print('Error performing a calculation on the CPU:', e)
